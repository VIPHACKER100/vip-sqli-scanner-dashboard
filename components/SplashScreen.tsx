...
const SplashScreen: React.FC<SplashScreenProps> = ({ onComplete }) => {
    ...
    return (
        <div className="fixed inset-0 z-[9999] bg-white dark:bg-[#020617] flex flex-col items-center justify-center p-8 overflow-hidden font-technical transition-colors duration-700">
            ...
            <div className="absolute inset-0 scan-line-overlay opacity-[0.03] dark:opacity-10"></div>

            {/* Animated Background Orbs */}
            <div className="absolute top-1/4 -left-20 w-96 h-96 bg-primary-600/5 dark:bg-primary-600/10 rounded-full blur-[120px] animate-pulse"></div>
            <div className="absolute bottom-1/4 -right-20 w-80 h-80 bg-purple-600/5 dark:bg-purple-600/10 rounded-full blur-[100px] animate-pulse delay-700"></div>
            ...
            <div className="mt-32 w-full max-w-md space-y-6">
                ...
                <div className="flex justify-between items-center opacity-40 px-2 pt-2">
                    <div className="flex gap-8">
                        <Cpu size={16} className={progress > 20 ? 'text-primary-600 dark:text-primary-500' : 'text-slate-400 dark:text-gray-700'} />
                        <Database size={16} className={progress > 40 ? 'text-primary-600 dark:text-primary-500' : 'text-slate-400 dark:text-gray-700'} />
                        <Terminal size={16} className={progress > 60 ? 'text-primary-600 dark:text-primary-500' : 'text-slate-400 dark:text-gray-700'} />
                        <Activity size={16} className={progress > 80 ? 'text-primary-600 dark:text-primary-500' : 'text-slate-400 dark:text-gray-700'} />
                    </div>
                    <div className="flex items-center gap-2">
                        <Lock size={12} className="text-slate-400 dark:text-gray-600" />
                        <span className="text-[8px] font-black text-slate-400 dark:text-gray-700 uppercase tracking-widest">TLS_SECURED</span>
                    </div>
                </div>
            </div>
            ...
            <div className="fixed bottom-12 flex flex-col items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                    <Fingerprint size={14} className="text-slate-400 dark:text-gray-700" />
                    <p className="text-[10px] text-slate-400 dark:text-gray-700 font-black uppercase tracking-[0.4em] font-mono flex items-center gap-2">
